...
def write_docs(html, status):
    ...
    _cvr_js = (
        ...
        "if(!c)return;"
        "fetch('kpi_history.json?v='+Date.now()).then(function(r){return r.json();})"
        ".then(function(h){"
        "var l=h.slice(-30);"
        ...
        "ticks:{font:{size:9},color:'#2688c9',callback:function(v){return v+'%';}},"
        "grid:{display:false}};"
        "c.options.scales.y3={position:'right',display:false,min:0,suggestedMax:200,"
        "ticks:{font:{size:9},color:'#f59e0b',callback:function(v){return v+'%';}},"
        "grid:{display:false}};}"
        "document.querySelectorAll('.perf-kpi-row .perf-kpi-tile').forEach(function(t){"
        "var lbl=t.querySelector('.perf-kpi-label');"
        "if(!lbl)return;"
